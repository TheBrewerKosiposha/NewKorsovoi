...
router.get("/", (request, response) => {
    ...
    database.query(query, args, (error, result) => {
        if(error){
            ...
            <li><a href="/users/registers">Создать нового пользователя</a></li>
                <h1>Пользователи:</h1>
                <ul>
                    ${users.map(user => `
                    <li>
...
router.get('/registers', (req, res) => {
    res.send(`
    <form method="POST" action="/users/registers" enctype="multipart/form-data">
      <label>Name:</label>
...
// Insert User
router.post("/registers",uploadImage.single('image'), (request, response) => {
    const name = request.body.name
    const email = request.body.email 
    const password = request.body.password
    var gender = request.body.gender
...
router.put("/info", checkAuth, (request, response) => {
    ...
    const query = "call updateUserPasswordByID(?,?)"    
